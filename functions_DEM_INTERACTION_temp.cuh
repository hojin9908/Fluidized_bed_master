#define HEAT_FLUX	5.0e4		// [W/m^2]
#define nop_hf		153			// number of DEM particles on heat flux surface
#define qv			1.0e6

////////////////////////////////////////////////////////////////////////
__global__ void KERNEL_DEM_interaction3D(int_t inout,int_t*g_str,int_t*g_end,part1*P1,part1*TP1,part3*P3,Real ttime)
{
	...
	Real tempi, temp1i, temp2i, temp3i, kci, kc1_i, kc2_i, kc3_i, kc0_i, kcavg_i, alph0_i, alph1_i, alph2_i, alph3_i, alphavg_i, diff_kc0_i, diff_kc1_i, diff_kc2_i,diff_kc3_i, cpi, emi, CTE;
	
	//Real search_range;
	Real tmpx,tmpy,tmpz,tmptqx,tmptqy,tmptqz,tmpn,tmpd,tmpdt,tmpdt1,tmpdt2,tmpdt3,temp_avg;	// tmptq : temporary value for torque calculation
...
ptypei=P1[i].p_type;
	...
	if(k_con_solve){
		...
		kc0_i=conductivity_1(tempi,ptypei,1);
		alph0_i=diffusivity_1(tempi,ptypei,1);
		diff_kc0_i=diff_conductivity_1(tempi,ptypei,1);

		kc1_i=conductivity_1(temp1i,ptypei,0);
		alph1_i=diffusivity_1(temp1i,ptypei,0);
		diff_kc1_i=diff_conductivity_1(temp1i,ptypei,0);

		kc2_i=conductivity_1(temp2i,ptypei,0);
		alph2_i=diffusivity_1(temp2i,ptypei,0);
	    diff_kc2_i=diff_conductivity_1(temp2i,ptypei,0);

		kc3_i=conductivity_1(temp3i,ptypei,0);
		alph3_i=diffusivity_1(temp3i,ptypei,0);
		diff_kc3_i=diff_conductivity_1(temp3i,ptypei,0);
		//cpi=heat_capacity(tempi,ptypei);
		emi=emissivity(tempi,ptypei);

		CTE=Effective_CTE(temp_avg,ptypei);

		kcavg_i=conductivity_1(temp_avg,ptypei,0);
		alphavg_i=diffusivity_1(temp_avg,ptypei,0);
	}
	...
	//tmpdt1=(kc1_i/(rhoi*cpi))*((2*A-1)/(dr1*2.5*dr1)+(2*A-3)/(dr1*dr1))*temp1i+(kci/(rhoi*cpi))*(-1/(dr1*2.5*dr1)+1/(dr1*dr1))*temp2i+(kci/(rhoi*cpi))*((2*B)*1/(dr1*2.5*dr1)+(2*B)/(dr1*dr1))*400+qv/(rhoi*cpi);
	//tmpdt2=(kc2_i/(rhoi*cpi))*(1/(dr1*1.5*dr1)+1/(dr1*dr1))*temp1i+(kci/(rhoi*cpi))*(-2/(dr1*dr1))*temp2i+(kci/(rhoi*cpi))*(-1/(dr1*1.5*dr1)+1/(dr1*dr1))*temp3i+qv/(rhoi*cpi);
	//tmpdt3=(kc3_i/(rhoi*cpi))*(1/(dr1*0.5*dr1)+1/(dr1*dr1))*temp2i+(kci/(rhoi*cpi))*(-1/(dr1*0.5*dr1)-1/(dr1*dr1))*temp3i+qv/(rhoi*cpi);
	...
	tmpdt1=diff_kc1_i/(kc1_i/alph1_i)*((2*temp_star-temp1i-temp2i)/(2*dr1))*((2*temp_star-temp1i-temp2i)/(2*dr1))+(2*alph1_i/(2.5*dr1))*(2*temp_star-temp1i-temp2i)/(2*dr1)+alph1_i*(2*temp_star-3*temp1i+temp2i)/(dr1*dr1)+qv/(kc1_i/alph1_i);
	tmpdt2=diff_kc2_i/(kc2_i/alph2_i)*((temp1i-temp3i)/(2*dr1))*((temp1i-temp3i)/(2*dr1))+2*(alph2_i/(1.5*dr1))*(temp1i-temp3i)/(2*dr1)+alph2_i*(temp1i-2*temp2i+temp3i)/(dr1*dr1)+qv/(kc2_i/alph2_i);
	tmpdt3=diff_kc3_i/(kc3_i/alph3_i)*((temp2i-temp3i)/(2*dr1))*((temp2i-temp3i)/(2*dr1))+2*(alph3_i/(0.5*dr1))*((temp2i-temp3i)/(2*dr1))+alph3_i*(temp2i-temp3i)/(dr1*dr1)+qv/(kc3_i/alph3_i);
				...
				if(g_str[k]!=cu_memset){
					...
					for(int_t j=g_str[k];j<fend;j++){
						...
						if (P1[j].p_type>1000){
							...
							if((tdist<3.25*radi)&&(tdist>0.0)){			// 2*sqrt(3)*rad -- should be modified -- assumption ybj
								...
								if (k_con_solve){
									...
									//emij_inv=(1-emi)/emi+(1-emj)/emj*ar_ratio+(1+ar_ratio-2.0*F_ij)/(ar_ratio-F_ij*F_ij);

									//tmpdt+=-10.0*sb_const*(tempi*tempi*tempi*tempi-tempj*tempj*tempj*tempj)*ar_i/(emij_inv*(kcavg_i/alphavg_i)*4/3*3.141592*radi*radi*radi);
									
									Y_ij=F_ij+(1-F_ij)*(ar_ratio/(1+ar_ratio));
									emij_inv=(1-emi)/(emi*ar_i)+(1-emj)/(emj*ar_j)+1/(Y_ij*ar_i);
									
									tmpdt=tmpdt-sb_const*(tempi*tempi*tempi*tempi-tempj*tempj*tempj*tempj)/(emij_inv*(kcavg_i/alphavg_i)*4/3*3.141592*radi*radi*radi);
							...
							if(tdist>0&&ovlp>0){
								...
								if(k_con_solve){
									Real kcj, kc_ij;
									kcj=conductivity_1(tempj,ptypej,1);
									kc_ij=1/(1/kc0_i+1/kcj);
									//tmpdt+=-4.0*kc_ij*sqrt(radij*ovlp)*(tempi-tempj);
									tmpdt+=-4.0*kc_ij*sqrt(2*radij*ovlp-ovlp*ovlp)*(tempi-tempj)/((kcavg_i/alphavg_i)*4/3*3.141592*radi*radi*radi);
								}
								...
								rcxi = (radi-0.5*ovlp)*nx;		// r vector (from center of particle i to contact point C)
								rcyi = (radi-0.5*ovlp)*ny;	
								rczi = (radi-0.5*ovlp)*nz;	
								
	
								rcxj = -(radj-0.5*ovlp)*nx;		// r vector (from center of particle j to contact point C)
								rcyj = -(radj-0.5*ovlp)*ny;
								rczj = -(radj-0.5*ovlp)*nz;
								...
								Real C_spr = (1.33333)*ymij*sqrt(radij)*sqrt(ovlp)*ovlp/mi;
								Real C_spr_s = 8.0*Gij*sqrt(radij*ovlp)/mi;
	
								Real Kn_dmp = 2*ymij*sqrt(radij*ovlp);
	
								Real K_dmp = sqrt(mij*Kn_dmp);
								Real C_dmp = sqrt(3.333333333)*0.0335184*K_dmp/mi;			//e=0.9
								//Real C_dmp = sqrt(3.333333333)*0.09119*K_dmp/mi;				//e=0.75
	
								Real K_dmp_s = sqrt(mij*C_spr_s*mi);
								Real C_dmp_s = sqrt(3.333333333)*0.0335184*K_dmp_s/mi;		//e=0.9
								...
								fsfx=0.3*C_spr*sx;				// tangential force (should be modifed 2019.10.25)
								fsfy=0.3*C_spr*sy;
								fsfz=0.3*C_spr*sz;
	...
	if(k_con_solve){
		if ((demidxi>k_num_part)&&(demidxi<=2*k_num_part)){					// heat flux (top)
			P3[i].dtemp=tmpdt-HEAT_FLUX*(L1-L2)*(L1-L2)/(nop_hf*(kcavg_i/alphavg_i)*4/3*3.141592*radi*radi*radi);	
			//P3[i].dtemp=tmpdt-HEAT_FLUX*(L1-L2)*(L1-L2)/(nop_hf*(kcavg_i/alphavg_i)*4/3*3.141592*radi*radi*radi);		
			//P3[i].dtemp=HEAT_FLUX*(L1-L2)*(L1-L2)/nop_hf;

		} 
		else if (demidxi>2*k_num_part) {				// constant temperature (bottom)
			 P3[i].dtemp=0.0;
		}
		else {
		...
		P3[i].dtemp2=tmpdt2;
		P3[i].dtemp3=tmpdt3;
		P1[i].rad=radi*(1+(0.00000000000003*temp_avg*temp_avg+0.00000000167*temp_avg+0.000001826)*50*(tmpdt+tmpdt1+tmpdt2+tmpdt3)/4*k_dt);
	}
